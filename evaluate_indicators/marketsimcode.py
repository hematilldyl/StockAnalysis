...
def compute_portvals(orders=None,
    ...
    prices[:, -1] = 1
    inc = 0
    ticker_data = get_data(['JPM'], pd.date_range(start_date, end_date))
    prices[:,0] = ticker_data['JPM']
    prices = pd.DataFrame(index=reference.index, data=prices, columns=['JPM','Cash'])
    trades = prices.copy()
